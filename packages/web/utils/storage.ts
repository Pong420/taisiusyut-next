...
export interface IStorage<T> {
  ...
  get<K extends keyof T>(prop: K): T[K] extends undefined ? T[K] | undefined : T[K];
...
function createMemoryStorage<T extends {}>(key: string, defaultValue: T): IStorage<T> {
...
function createStorageFromStorage(storage: IStorage<Record<string, any>>) {
  return <T>(key: string, defaultValue: T): IStorage<T> => {
    let value = (storage.get(key) || JSON.parse(JSON.stringify(defaultValue))) as T;
    return {
      key,
      get: <K extends keyof T>(prop?: K, fallback?: T[K]) => {
        if (typeof prop === 'undefined') {
...
export const createLocalStorage = storageSupport() ? createStorageFromWebStorage(localStorage) : createMemoryStorage;

export const createSessionStorage = storageSupport()
  ? createStorageFromWebStorage(sessionStorage)
  : createMemoryStorage;

const createTaisiusyutStorage = createStorageFromStorage(createLocalStorage('taisiusyut', {} as Record<string, any>));

export const createAdminStorage = createStorageFromStorage(createTaisiusyutStorage('admin', {} as Record<string, any>));

// admin
export const createChapterSotrage = createStorageFromStorage(createAdminStorage('chapter', {} as Record<string, any>));

export const createClientStorage = createStorageFromStorage(
  createTaisiusyutStorage('client', {} as Record<string, any>)
);

export const lastVisitStorage = createClientStorage('lasvisit', {} as Record<string, number>);
