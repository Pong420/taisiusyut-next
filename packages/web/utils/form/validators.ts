...
export const number: Validator = (_, value) =>
  !value || /^-?\d+\.?\d*$/.test(value) ? Promise.resolve() : Promise.reject('Plase input number only');

export const integer =
  (msg = 'Plase input integer only'): Validator =>
  (_, value) =>
    value === '' || isNaN(Number(value)) || /^(-)?\d*$/.test(value) ? Promise.resolve() : Promise.reject(msg);

export const maxDecimal =
  (max: number): Validator =>
  (_, value) =>
    value === '' || isNaN(Number(value)) || new RegExp(`^(\\d+|\\d+\\.\\d{0,${max}})$`).test(value)
      ? Promise.resolve()
...
export const minLength = lengthComparation((length, minLength) => length >= minLength);

export const maxLength = lengthComparation((length, maxLength) => length <= maxLength);
...
export const regex =
  ...
  (_, value) => {
    const valid = typeof value === 'string' ? regex.test(value) : Promise.reject(`value is not a string`);
    return valid ? Promise.resolve() : Promise.reject(msg);
  };
