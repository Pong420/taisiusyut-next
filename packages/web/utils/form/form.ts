...
export interface FieldData<S extends {} = Store, Name = NamePath<S>> extends Partial<Omit<Meta, 'name'>> {
  name: Name;
  value?: Name extends Paths<S> ? PathType<S, Name> : Name extends keyof S ? S[Name] : undefined;
}
...
type OmititedRcFieldProps = Omit<RcFieldProps, 'name' | 'dependencies' | 'children' | 'rules'>;

export interface FormItemLabelProps extends HTMLDivProps {
  label?: ReactNode;
}

interface BasicFormItemProps<S extends {} = Store> extends OmititedRcFieldProps {
  name?: NamePath<S>;
...
export type FormItemProps<S extends {} = Store> = BasicFormItemProps<S> &
  FormItemPropsDeps<S> &
  Pick<FormGroupProps, 'label' | 'inline'>;
...
const getValues = (obj: any, paths: (string | number)[]) => paths.reduce((result, key) => result && result[key], obj);
...
export function createForm<S extends {} = Store, V = S>({
  ...
  const FormItem = (itemProps: FormItemProps<S>) => {
    ...
    return React.createElement(
      ...
      (control: Control<unknown>, { touched, validating, errors }: FieldData<S>, form: FormInstance<S>) => {
        ...
        return React.createElement<FormGroupProps>(
          FormGroup,
          {
            label,
            inline,
          ...
          childNode,
          React.createElement<HTMLDivProps>('div', { className: ClassNames.help }, error)
        );
      }
    );
  };

  const Form = React.forwardRef<FormInstance<S>, FormProps<S, V>>(function FormBase(
    {
      layout = 'vertical',
      className = '',
      children,
    ...
    return React.createElement(
      RcForm,
      {
        ...
        initialValues:
          initialValues && transoformInitialValues ? transoformInitialValues(initialValues) : initialValues,
        onFinish:
          onFinish &&
          ((store: unknown) => {
