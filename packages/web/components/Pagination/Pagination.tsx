...
type Actions = { type: 'PAGE_CHANGE'; payload: number } | { type: 'INIT'; payload: Partial<InitialState> };
...
export const Pagination = React.memo<PaginationProps>(({ pageNo = 1, pageSize = 10, total = 0, onPageChange }) => {
  const [state, dispatch] = useReducer(reducer, { currentPage: pageNo, total, pageSize }, getState);
  ...
  return (
    <div className={classNames.pagiation}>
      <ButtonGroup>
        ...
        />
        <Button disabled={state.currentPage === state.totalPages} onClick={() => changePage(state.totalPages)}>
          Last
        </Button>
      </ButtonGroup>
    </div>
