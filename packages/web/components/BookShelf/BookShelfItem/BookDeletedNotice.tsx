...
const onFailure = Toaster.apiError.bind(Toaster, `Remove book from shelf failure`);
...
export function BookDeletedNotice({ id, onSuccess: _onSuccess }: Props) {
  ...
  const [{ loading }, { fetch }] = useRxAsync(removeBookFromShelf, {
    defer: true,
    onSuccess,
    onFailure
  });
  ...
  return (
    <div className={classes['deleted']}>
      <div>
        <Icon iconSize={28} icon="warning-sign" className={classes['deleted-icon']} />
        <div>書籍已被刪除</div>
        <Button small intent="primary" text="確認" loading={loading} onClick={() => fetch(id)} />
      </div>
    </div>
  );
}
