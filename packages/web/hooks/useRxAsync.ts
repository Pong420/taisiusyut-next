...
type Actions<I, P> = RxAsyncInit<P> | RxAsyncSuccess<I> | RxAsyncFailure | RxAsyncCancel | RxAsyncReset;
...
export type RxAsyncOptions<I> = {
  ...
  mapOperator?: typeof switchMap | typeof concatMap | typeof exhaustMap | typeof mergeMap | typeof flatMap;
};
...
export interface RxAsyncActionsWithParam<I, P> extends RxAsyncActionsBase<I, P> {
  fetch: F1<P, void>;
}

export interface RxAsyncActionsOptionalParam<I, P> extends RxAsyncActionsBase<I, P> {
  fetch: F2<P, void>;
}

export type RxAsyncState<I> = State<I>;

export type RxAsyncActions<I, P> = RxAsyncActionsWithParam<I, P> | RxAsyncActionsOptionalParam<I, P>;
...
export function useRxAsync<I, P>(
  ...
  const [state, dispatch] = useReducer<Reducer<State<I>, Actions<I, P>>, Partial<State<I>> | undefined>(
    reducer,
    { loading: !defer },
    initializer
  );
  ...
  const actions = useMemo<RxAsyncActions<I, P>>(() => {
    return {
      fetch: (payload?: P) => actionRef.current.next({ type: 'FETCH_INIT', payload }),
  ...
  useEffect(() => {
    const action$ = <A extends Actions<I, P>>(...types: A['type'][]) =>
      actionRef.current.pipe(filter((action): action is A => types.includes(action.type)));
      ...
      .pipe(
        mapOperator(action =>
          ...
          RxDefer(() => request(action.payload!)).pipe(
            ...
            catchError(payload => of<RxAsyncFailure>({ type: 'FETCH_FAILURE', payload })),
