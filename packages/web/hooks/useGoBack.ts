...
export function GoBackProvider({ children }: Props) {
      ...
      goBack: async ({ targetPath = [] }) => {
        ...
        if (
          ...
          (targetPaths.length === 0 ||
            targetPaths.some(path => isMatchUrl(path, decodeURIComponent(previous).replace(/\?.*/, ''))))
        ) {
  ...
  useEffect(() => {
    const handler = (url: string) => {
      ...
      const maxRecords = 5;
      const samePathname = !!last && last.replace(/\?.*/, '') === url.replace(/\?.*/, '');
  ...
  return React.createElement(ActionContext.Provider, { value: actions }, children);
}
