...
export function withBreakPoint<P>(Component: React.ComponentType<P>, { validate, fallback = null }: WithBreakPointOps) {
...
export function BreakPointsProvider({ children }: { children: React.ReactElement }) {
  const [breakPoint, setBreakPoint] = useState<BreakPoint>(
    typeof window !== 'undefined' ? getBreakPoint(window.innerWidth) : 1280
  );
  const [onResize] = useState(() => ([entry]: ResizeEntry[]) => {
