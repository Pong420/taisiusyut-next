...
> = (state: CRUDState<I, Prefill>, action: CRUDActions<I, K, M>) => CRUDState<I, Prefill>;

export interface CreateCRUDReducerOptions<Prefill extends boolean = true, M extends CRUDActionTypes = CRUDActionTypes> {
  prefill?: Prefill;
  actionTypes?: M;
...
export function createPlaceholder<I, K extends Key<I>>(
  ...
  return {
    ...
    list: Array.from({ length }, (_, index) => ({ [key]: keyGenerator(index) } as unknown as Partial<I>))
  };
}
...
>(
  ...
  const reducer: CRUDReducer<I, K, Prefill, M> = (state = defaultState, action) => {
    if (isAction(actionTypes, action, 'PAGINATE')) {
      return (() => {
        const { data, pageNo, total, pageSize = state.pageSize } = parsePaginatePayload(action.payload);
    ...
    if (isAction(actionTypes, action, 'LIST')) {
      return (action as List<'', I>).payload.reduce(
        (state, payload) => reducer(state, { type: actionTypes['CREATE'], payload }),
    ...
    if (isAction(actionTypes, action, 'UPDATE')) {
      ...
      return index === -1
        ...
        : {
            ...
            list: [...state.list.slice(0, index), updated, ...state.list.slice(index + 1)]
          };
    }
    ...
    if (isAction(actionTypes, action, 'PARAMS')) {
      ...
      const toNum = (value: unknown, num: number) =>
        typeof value === 'undefined' || isNaN(Number(value)) ? num : Number(value);
