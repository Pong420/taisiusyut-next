...
export type GetCreatorsAction<T extends Record<string, (...args: any[]) => any>> = ReturnType<T[keyof T]>;

export type CRUDActionType = 'LIST' | 'CREATE' | 'UPDATE' | 'DELETE' | 'INSERT' | 'PAGINATE' | 'PARAMS' | 'RESET';

export type CRUDActionTypes<Type extends string = any> = {
  [K in CRUDActionType]?: Type;
};

export type CustomActionTypes<M extends Partial<CRUDActionTypes> = CRUDActionTypes> = M &
  Omit<typeof DefaultCRUDActionTypes, keyof M>;

export type UpdatePayload<I, K extends Key<I>> = Partial<I> & { [T in K]: string };
...
export type CRUDActionCreators<I, K extends Key<I>, M extends CRUDActionTypes = CRUDActionTypes> = {
  list: (payload: I[]) => List<M['LIST'], I>;
  create: (payload: I) => Create<M['CREATE'], I>;
  insert: (payload: I, index?: number) => Insert<M['INSERT'], I>;
  update: (payload: Update<string, I, K>['payload']) => Update<M['UPDATE'], I, K>;
...
export type CRUDActions<I, K extends Key<I>, M extends CRUDActionTypes = CRUDActionTypes> = GetCreatorsAction<
  CRUDActionCreators<I, K, M>
>;
